...
class CurlDuolingo:
    ...
    def _send_request_curl(self, method, url, data=None, retries=3):
        # Sử dụng session dùng chung, không tạo mới mỗi lần
        try:
            if method.upper() == 'GET':
            ...
            if retries > 1:
                time.sleep(0.05)  # Giảm thời gian chờ retry từ 0.2s xuống 0.05s
    ...
    def buff_streak(self, target_days: int, progress_callback=None) -> int:
        ...
        while saved < target_days and not self._should_stop():
            try:
                    ...
                    if not res or not res.get("id"):
                        ...
                        if consecutive_fail > 10:
                            ...
                            try:
                                ...
                                pass
                            continue
                        time.sleep(0.1)  # Giảm sleep từ 0.3s xuống 0.1s
                        continue
                ...
                # ==== TĂNG TỐC: Giảm sleep xuống rất nhỏ (0.002 - 0.005s) thay vì 0.02 - 0.05s ====
                ...
                if consecutive_fail > 10:
                    ...
                    try:
                        ...
                        pass
                time.sleep(0.05)  # Giảm sleep lỗi từ 0.1s xuống 0.05s
                continue
