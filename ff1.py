...
class AsyncDuolingo:
    ...
    async def farm_xp(self, target_xp: int, progress_callback=None) -> Tuple[int, float, float]:
            ...
            async def logger_task():
                ...
                while True:
                    ...
                    if time.time() - last_log > 1:
                        logger.info(f"⚡ XP: {curr_earned:,}/{target_xp:,} | Speed: {speed:.1f} XP/s | Success: {success_rate:.1f}%")
                        last_log = time.time()
                    if curr_earned >= target_xp:
                        break
        ...
        if actual_gained > 0:
            logger.info(f"📊 XP thực tế nhận: +{actual_gained:,} (farm: {total_earned:,})")
            total_earned = max(total_earned, actual_gained)

        self.success_rate = (self.successful_attempts / self.total_attempts * 100) if self.total_attempts > 0 else 0
            ...
            try:
                ...
                for attempt in range(3):
                    ...
                    if status_code == 200 and res and res.get("id"):
                        ...
                        rate_limit_wait = 2.0 + random.uniform(0, 1.0)
                        break
                    else:
                        simple_payload = {
                ...
                if status_code != 200 or not res or not res.get("id"):
                    ...
                    if consecutive_create_fail > 10:
                        logger.warning("⚠️ Quá nhiều lỗi tạo session, tạm dừng 10s...")
                        time.sleep(10)
                        consecutive_create_fail = 0
                    else:
                        time.sleep(0.2)
                ...
                if complete_success:
                    ...
                    if consecutive_fail > 10:
                        logger.warning("⚠️ Quá nhiều lỗi hoàn thành session, tạm dừng 10s...")
                        time.sleep(10)
                        consecutive_fail = 0
                    else:
                        time.sleep(0.2)
