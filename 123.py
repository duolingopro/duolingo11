...
# FULL CODE - FIX STREAK (ổn định 0.3 streak/s) + GEMS (selenium) + PROXY + REALTIME PROGRESS + STOP + CONNECTION POOL
# ================================================================
...
# ================== LỚP CURL_DUOLINGO (dùng Selenium cho gems, curl_cffi cho streak ổn định) ==================
class CurlDuolingo:
    def __init__(self, token: str):
        self.token = token
        self.uid = get_user_id_from_token(token)
    ...
    # ---------- curl_cffi request (dùng cho streak) ----------
    def _send_request_curl(self, method, url, data=None, retries=2):
        with requests.Session() as session:
            session.headers.update(self.headers)
            try:
                if method.upper() == 'GET':
                    ...
                    resp = session.put(url, json=data, timeout=3)
                else:
                    return None
                if resp.status_code == 200:
                    return resp.json()
    ...
    # ================== BUFF STREAK (Ổn định, tốc độ ~0.3 streak/s) ==================
    def buff_streak(self, target_days: int, progress_callback=None) -> int:
        ...
        # Vòng lặp đơn luồng, mỗi lần sleep ~2.5-3.5s để đạt 0.3 streak/s
        while saved < target_days and not self._should_stop():
            try:
                ...
                res = self._send_request_curl('POST', f"{BASE_URL}/{API_VERSION}/sessions", session_payload)
                if not res or not res.get("id"):
                    ...
                    res = self._send_request_curl('POST', f"{BASE_URL}/{API_VERSION}/sessions", simple_payload)
                    if not res or not res.get("id"):
                        consecutive_fail += 1
                        # Nếu fail quá nhiều, nghỉ lâu hơn
                        if consecutive_fail > 10:
                            time.sleep(5)
                            consecutive_fail = 0
                            # Refresh driver nếu cần
                            try:
                                driver = get_selenium_driver()
                                driver.refresh()
                                time.sleep(0.5)
                ...
                complete = self._send_request_curl('PUT', f"{BASE_URL}/{API_VERSION}/sessions/{sess_id}", complete_payload)
                if complete and complete.get('status') != 'error':
                    saved += 1
                    consecutive_fail = 0
                    last_success_time = time.time()
                    if progress_callback:
                    ...
                    complete_alt = self._send_request_curl('PUT', f"{BASE_URL}/{API_VERSION}/sessions/{sess_id}", alt_payload)
                    if complete_alt and complete_alt.get('status') != 'error':
                        saved += 1
                        consecutive_fail = 0
                        last_success_time = time.time()
                        if progress_callback:
                ...
                # Điều chỉnh sleep để đạt ~0.3 streak/s: trung bình 3.3s mỗi ngày
                # Thời gian mỗi request khoảng 0.2-0.5s, sleep thêm 2.8-3.3s
                sleep_time = random.uniform(2.8, 3.5)
            ...
            # Reset driver sau mỗi 200 ngày
            if saved > 0 and saved % 200 == 0:
                try:
                    driver = get_selenium_driver()
                    driver.refresh()
                    time.sleep(0.5)
...
# ================== LỆNH SLASH ==================
@bot.tree.command(name="farmxp", description="⚡ Cày XP (tốc độ theo cấp bậc)")
@app_commands.describe(xp="Số XP muốn farm (tối đa 5.000.000)")
async def farmxp(interaction: discord.Interaction, xp: int = 100000):
    try:
        await interaction.response.defer()
    except:
    ...
    if not acc:
        ...
        return
    if not (1 <= xp <= 5000000):
        await interaction.followup.send("⚠️ XP từ 1 đến 5.000.000.")
        return
...
@bot.tree.command(name="buffstreak", description="🔥 Buff Streak")
@app_commands.describe(count="Số ngày muốn buff (1-1000000)")
async def buffstreak(interaction: discord.Interaction, count: int = 1000):
    try:
        await interaction.response.defer()
    except:
        await interaction.response.send_message("⚠️ Interaction đã hết hạn.", ephemeral=True)
...
async def stop_task(interaction: discord.Interaction):
    ...
    if user_id in user_tasks and user_tasks[user_id]:
        ...
        # Đặt stop flag cho các task đang chạy trong thread pool
...
async def help_cmd(interaction: discord.Interaction):
    ...
    embed.add_field(name="⚡ Cày XP", value="`/farmxp xp:<số>` (max 5.000.000)", inline=False)
    embed.add_field(name="💎 Farm Gems", value="`/farmgems gems:<số>` (max 50.000)", inline=False)
    embed.add_field(name="🔥 Buff Streak", value="`/buffstreak count:<số>` (CRE không giới hạn)", inline=False)
